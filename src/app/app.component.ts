...
export class AppComponent implements OnInit, OnDestroy {
  ...
  initCharacterEvents(): void {
    ...
    this.charactersResults$ = this.searchTermByCharacters.pipe(
      ...
      debounceTime(400),
      switchMap((inpuValue: string) =>
        this.mockDataService.getCharacters(inpuValue)
      )
      // YOUR CODE ENDS HERE
    );
  }
  ...
  initLoadingState(): void {
    ...
    // YOUR CODE STARTS HERE
    const charactersLoader$: Observable<boolean> =
      this.mockDataService.getCharactersLoader();
    const planetsLoader$: Observable<boolean> =
      this.mockDataService.getPlanetLoader();

    const loadingSubscription: Subscription = combineLatest([
      charactersLoader$,
      planetsLoader$,
